...
# Commits whose first line starts with this prefix are excluded from P10Y / phase breakdown
...
def generate_phase_breakdown(
...
) -> Dict[Optional[int], Estimation]:
    """
    Aggregate estimation metrics per implementation-plan phase.

    Each commit is attributed to its phase (parsed from the `pNN_` subject prefix). Commits
    with no phase prefix are bucketed under None ("unphased").

    Args:
        metadata: Code generation metadata with per-commit phase numbers.
        commit_stats_data: P10Y commit statistics data.

    Returns:
        Mapping of phase number (or None for unphased) to aggregated Estimation.
    """
    commit_stats_map = {
    ...
    phase_commits: Dict[Optional[int], List[Dict[str, Any]]] = {}
    ...
    for commit_info in metadata.commits:
        ...
        phase_commits.setdefault(commit_info.phase, []).append(commit_stats_map[commit_sha])

    phase_estimations: Dict[Optional[int], Estimation] = {}
    for phase, stats_list in phase_commits.items():
        estimation = Estimation.prepare_for_estimation()
        for commit_stats in stats_list:
            estimation = extract_estimation_from_commit_stats(estimation, commit_stats)
        phase_estimations[phase] = estimation

    return phase_estimations
...
def _parse_phase_from_subject(subject: str, logger: logging.Logger) -> Optional[int]:
    """
    Parse the implementation-plan phase from a `pNN_<message>` commit subject (first line).

    The `pNN_` prefix is injected deterministically by the prepare-commit-msg git hook
    (see app/utils/workspace_git_hooks.py), so a well-formed generation run always carries
    it. Commits without a `pNN_` prefix (deploy, strays) return None and are bucketed as
    "unphased" downstream.
    """
    s = (subject or "").strip()
    if "_" not in s:
        return None
    token, _rest = s.split("_", 1)
    token = token.strip().lower()
    if len(token) < 2 or token[0] != "p" or not token[1:].isdigit():
        logger.debug(
            "Commit subject has no pNN_ phase prefix: %s — using [unphased]", s[:80]
        )
        return None
    return int(token[1:])
...
def build_code_generation_metadata_from_git(
    ...
    for sha, subject in rows:
        if _subject_excluded_from_estimation(subject, logger):
            ...
            continue
        phase = _parse_phase_from_subject(subject, logger)
        commits.append(
            CommitInfo(sha=sha, message=subject, phase=phase),
        )
...
class FilteredCommitStats(NamedTuple):
    """Result of intersecting P10Y rows with the local git allowlist."""
    rows: List[dict]        # commits with a valid technology (used for scoring)
    returned_count: int     # commits P10Y returned for our allowlist, before the valid-tech filter
...
async def fetch_and_filter_commit_stats(
    ...
    Returns:
        FilteredCommitStats: the valid-technology rows used for scoring, plus the count of
        commits P10Y returned for our allowlist (before the valid-technology filter) so
        coverage can distinguish a P10Y service gap from non-code commits filtered by tech.
    """
    ...
    return FilteredCommitStats(
        rows=filtered_commit_stats_data,
        returned_count=len(hash_filtered_commits),
    )